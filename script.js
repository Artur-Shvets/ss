...
function deleteElement () {
  // Getting list Of Delete
  listOfHierarchies.forEach(function(branch, i, listOfHierarchies) {
    if (listOfHierarchies.length > 1 & branch[0] == hitIndex) {
      listOfDelete = branch;
      listOfHierarchies.splice(i, 1);
    }
  });
  // Delete Elements
  listOfDelete.forEach(function (deleteIndex, i, listOfDelete) {
    // Delete Hierarchies
    listOfHierarchies.forEach(function(branch, i, listOfHierarchies) {
      branch.forEach(function (index, i, branch) {
    ...
    // Delete Links
    listOfLinks.forEach(function (link, i, listOfLinks) {
      if (link.index1 == deleteIndex || link.index2 == deleteIndex) {
        listOfLinks.splice(i, 1);
      };
    });
    // Delete Blocks and Features
    listOfBlocks.forEach(function (block, i, listOfBlocks) {
      if (i == deleteIndex) {
        delete listOfBlocks[i];
        delete listOfFeatures[i];
      };
    });
  });
  activeBlocks = false;
